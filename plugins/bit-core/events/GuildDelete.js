    ...
    async execute(guild) {
        ...
        if(dev_mode === true) {
            if(guild.available === true) {
                bit.log(3, "Bit", true, locale.debug.dev_mode.guild.remove.replace('{guildname}', guild.name).replace('{guildid}', guild.id))
                //console.log(locale.debug.dev_mode.guild.remove.replace('{guildname}', guild.name).replace('{guildid}', guild.id));
            } else {
                bit.log(3, "Bit", true, locale.debug.dev_mode.guild.removeunavailable.replace('{guildname}', guild.name).replace('{guildid}', guild.id))
